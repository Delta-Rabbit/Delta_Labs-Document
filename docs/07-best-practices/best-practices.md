...
- **Use the component library** — build UI from [Design System](../02-design-system/design-system.md) components and tokens; avoid one-off inline styles or arbitrary values.
- **Props and context** — prefer clear props and documented context over deep prop drilling or global mutable state.

### 1.2 Hooks and State

- **Custom hooks** — prefix with `use`, return objects for multiple values. See [Coding Standards → Custom Hooks](../03-coding-standards/coding-standards.md#7-custom-hooks).
...
- **Rate limiting** — apply rate limits per user/IP on auth, AI, and mutation endpoints; see [API Reference](../06-api-reference/api-reference.md#-rate-limiting).
...
Use your project’s test runner and conventions (e.g. Jest, Vitest) and align with [Coding Standards](../03-coding-standards/coding-standards.md).
