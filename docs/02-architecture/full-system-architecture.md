...
Key decisions (e.g. MongoDB only, modular monolith, AI tools → domain services) are reflected in this doc. For a separate ADR log, maintain a list in the repo (e.g. `docs/architecture-decisions/`) and link here.
...
- [Development Standards](/docs/04-development-standards/standards-intro) — cross-cutting patterns.
