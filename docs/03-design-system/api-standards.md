...
This document defines **API-related standards** for Delta Labs: how backends expose endpoints and how the frontend consumes them. It complements the [API Reference](../06-reference/api-reference/README), which describes specific endpoints and contracts.
...
- **Caching**: Use a consistent strategy (e.g. refetch on focus, SWR, or explicit "refresh") and align with [Delta Labs System Architecture](../02-architecture/full-system-architecture) (§4, §5).
...
- **Frontend**: On 429, show a "too many requests" message and back off before retrying; see [API Reference → Rate Limiting](../06-reference/api-reference/README#-rate-limiting).
...
- [API Reference](../06-reference/api-reference/README) — endpoints, auth, errors, rate limits.
- [Delta Labs System Architecture](../02-architecture/full-system-architecture) — API (§4.4) and data model (§5).
- [Security Overview](../08-security/security-overview) — auth and input safety.
